...
export function NewArrivals() {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'All') {
      const newOrFeatured = rawProducts.filter((p) => p.is_new || p.featured)
      filtered = newOrFeatured.length > 0 ? newOrFeatured : rawProducts
    }
      ...
      <div className="mx-auto max-w-[1440px] px-3 lg:px-10">
        <div className="flex flex-col gap-8 justify-between lg:flex-row lg:items-center lg:gap-20">
          <div className="max-w-xl">
            ...
            <h2 className="mt-4 text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[var(--text)] ">
            ...
            <div className="inline-flex min-w-max items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--bg-surface)] p-2">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`relative shrink-0 rounded-full px-6 py-2.5 font-mono text-[11px] uppercase tracking-widest transition-colors duration-300 ${activeTab === cat
